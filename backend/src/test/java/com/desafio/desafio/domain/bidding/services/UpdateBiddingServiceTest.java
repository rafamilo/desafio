...
public class UpdateBiddingServiceTest {

  @InjectMocks
  private UpdateBiddingService updateBiddingService;

  @Mock
  private BiddingRepository biddingRepository;

  @Test
  public void shouldBeImplementIUpdateBiddingService() {
    Class[] ifaces = UpdateBiddingService.class.getInterfaces();
    assertTrue(ifaces.length == 1);

    for (Class iface : ifaces) {
      assertTrue(iface.getName().contains("IUpdateBiddingService"));
    }
  }
  ...
  public void shouldToCallRepositoryFindAll() {
    ...
    updateBiddingService.update(bidding);
